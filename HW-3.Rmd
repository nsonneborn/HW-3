...
The model performed better on the test set by less than 1% of predictions. This may be due to the size of the data set.
...
gold_clipped <- filter(gold, as.numeric(year(Date)) >= 2014)
bitcoin_clipped <- filter(bitcoin, as.numeric(year(Date)) >= 2014)
ggplot() + 
  geom_line(data =bitcoin_clipped, aes(x = Date, y = Bid, col = "bicoin"))  + 
...
bitcoin1 <- bitcoin_clipped %>% mutate(percent_change = Delt(Bid)) # should I be looking at Bid, Ask, or 24 Average??
...
In the past couple of years, bitcoin has had more day-to-day price growth. The price of gold hasn't changed all that much, relatively speaking. A foreign currency exchanger might be more interested in bitcoin, based off of this analysis.
